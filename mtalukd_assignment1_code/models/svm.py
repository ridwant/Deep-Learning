...
class SVM:
    ...
    def calc_gradient(self, X_train: np.ndarray, y_train: np.ndarray) -> np.ndarray:
        ...
        score = prediction[np.arange(prediction.shape[0]), y_train].reshape(prediction.shape[0], 1)
        miss_score = (prediction - score) > -1 
        miss_score[range(miss_score.shape[0]), y_train] = False
        ...
        for ind,i in enumerate(miss_score):
            if not np.any(i):
                continue
            correct_class = y_train[ind]
            x = X_train[ind]
            for class_index, j in enumerate(i):
                if j == True:
                    correct_updates[correct_class] += (self.alpha * x)
                    incorrect_updates[class_index] -= (self.alpha * x)
                    incorrect_no_update[class_index][0] += 1
                    
            correct_no_update[correct_class][0] += 1
                
        gradiant = np.zeros((self.n_class, X_train.shape[1]))
        for index, i in enumerate(gradiant):
            if correct_no_update[index][0] != 0:
                self.bias[index]+=self.alpha
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray):
        ...
        for epoch in range(self.epochs):
          ...
          for batch in range(batch_size):
            ...
            self.w = self.w * (1 - (self.alpha * self.reg_const / X_train_copy.shape[0]))
            self.alpha = self.alpha * (1 / (1 + (decay * epoch)))
